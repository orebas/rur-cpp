...
struct SeparatingElementInfo {
    ...
    SeparatingElementInfo() : is_variable(false), variable_index(-1) {}
};
 ...
 */
bool try_separating_coefficients(
    ...
    for (int i = 0; i < num_variables && i < static_cast<int>(coeffs.size()); ++i) {
        ...
        ModularCoeff lambda_mod = (coeffs[i] < 0) 
            ? (prime - ((-coeffs[i]) % prime)) % prime 
            : coeffs[i] % prime;
...
compute_univariate_parameterization_with_hint(
    ...
    if (hint.is_variable && hint.variable_index > 0) {
        ...
        if (test_mp.success && test_mp.degree == quotient_basis.size()) {
            ...
            std::cout << "Hint didn't work (degree " << test_mp.degree 
    ...
    return compute_univariate_parameterization_with_retry(
        quotient_basis, i_xw, t_v, num_variables, prime, max_retries
    );
}
 ...
 */
SeparatingElementInfo extract_separating_element_info(
