...
inline ModularCoeff
modular_inverse(ModularCoeff a, ModularCoeff p) {
...
inline void
normalize_polynomial(std::vector<ModularCoeff> &poly) {
    while (poly.size() > 1 && poly.back() == 0) { poly.pop_back(); }
}

// Compute derivative of polynomial
inline std::vector<ModularCoeff>
polynomial_derivative(const std::vector<ModularCoeff> &poly, ModularCoeff prime) {
    if (poly.size() <= 1) {
    ...
    for (size_t i = 1; i < poly.size(); ++i) {
        ...
        deriv[i - 1] = static_cast<ModularCoeff>((static_cast<AccModularCoeff>(i) * poly[i]) % prime);
    }
...
inline std::vector<ModularCoeff>
polynomial_add(const std::vector<ModularCoeff> &a, const std::vector<ModularCoeff> &b, ModularCoeff prime) {
    size_t max_size = std::max(a.size(), b.size());
    std::vector<ModularCoeff> result(max_size, 0);

    for (size_t i = 0; i < a.size(); ++i) { result[i] = a[i]; }

    for (size_t i = 0; i < b.size(); ++i) { result[i] = (result[i] + b[i]) % prime; }
...
inline std::vector<ModularCoeff>
polynomial_subtract(const std::vector<ModularCoeff> &a, const std::vector<ModularCoeff> &b, ModularCoeff prime) {
    size_t max_size = std::max(a.size(), b.size());
    std::vector<ModularCoeff> result(max_size, 0);
...
inline std::vector<ModularCoeff>
polynomial_multiply(const std::vector<ModularCoeff> &a, const std::vector<ModularCoeff> &b, ModularCoeff prime) {
...
inline std::pair<std::vector<ModularCoeff>, std::vector<ModularCoeff>>
polynomial_divmod(std::vector<ModularCoeff> a, // Copy since we modify it
                  const std::vector<ModularCoeff> &b,
                  ModularCoeff prime) {
    normalize_polynomial(a);

    if (b.empty() || (b.size() == 1 && b[0] == 0)) { throw std::runtime_error("Division by zero polynomial"); }
    ...
    if (b.size() == 1) {
        ...
        for (size_t i = 0; i < a.size(); ++i) {
            quotient[i] = static_cast<ModularCoeff>((static_cast<AccModularCoeff>(a[i]) * b_inv) % prime);
        }
        return { quotient, { 0 } };
    }
    ...
    while (a.size() >= b.size() && !(a.size() == 1 && a[0] == 0)) {
        ...
        ModularCoeff q_coeff = static_cast<ModularCoeff>((static_cast<AccModularCoeff>(a.back()) * b_lead_inv) % prime);
...
inline std::vector<ModularCoeff>
polynomial_mod(const std::vector<ModularCoeff> &a, const std::vector<ModularCoeff> &b, ModularCoeff prime) {
...
inline PolynomialGCDResult
polynomial_extended_gcd(const std::vector<ModularCoeff> &a, const std::vector<ModularCoeff> &b, ModularCoeff prime) {
    // Initialize
    ...
    if (!old_r.empty() && old_r.back() != 0 && old_r.back() != 1) {
        ...
        for (auto &coeff : old_r) {
            coeff = static_cast<ModularCoeff>((static_cast<AccModularCoeff>(coeff) * lead_inv) % prime);
        }
...
inline std::vector<ModularCoeff>
polynomial_inverse_mod(const std::vector<ModularCoeff> &a, const std::vector<ModularCoeff> &m, ModularCoeff prime) {
...
inline std::vector<ModularCoeff>
polynomial_scalar_multiply(const std::vector<ModularCoeff> &poly, ModularCoeff scalar, ModularCoeff prime) {
    std::vector<ModularCoeff> result(poly.size());
    for (size_t i = 0; i < poly.size(); ++i) {
        result[i] = static_cast<ModularCoeff>((static_cast<AccModularCoeff>(poly[i]) * scalar) % prime);
    }
    normalize_polynomial(result);
    return result;
}
...
inline std::vector<ModularCoeff>
compute_rur_numerator(const std::vector<ModularCoeff> &f_t,          // f(t) from relation h(t)*x - f(t) = 0
    ...
    // Compute (f(t) * h(t)^(-1)) * T'(t) mod T(t), then trim to deg(T)-1
    auto result = polynomial_multiply(f_h_inv, t_prime, prime);
    result = polynomial_mod(result, minimal_poly, prime);
    if (result.size() > minimal_poly.size()) { result.resize(minimal_poly.size()); }
    normalize_polynomial(result);
