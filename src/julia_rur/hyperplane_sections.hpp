...
struct HyperplaneSectionConfig : public EnhancedSolverConfig {
    ...
    int min_coefficient = -10000;
    int max_coefficient = 10000;
    ...
    // Robust dimension analysis controls
    int dimension_num_primes = 2;      // How many primes to sample (reduce overhead)
    int dimension_slicing_retries = 2; // Hyperplane sets per s during slicing fallback

    // Force entering slicing even if a quick GB check says zero-dimensional.
    // Useful when the input has redundant equations that confuse the pre-check.
    bool force_slicing = false;
};
...
analyze_system_dimension(const std::vector<std::string> &polynomials,
                         ...
                         ModularCoeff prime = 0);  // 0 means use random prime

/**
 * @brief Robust dimension analysis across multiple primes with slicing fallback
 *
 * Runs analyze_system_dimension across several primes and returns the
 * majority dimension when stable. If unstable, uses hyperplane slicing
 * to determine the smallest number of random hyperplanes needed to
 * obtain a zero-dimensional system; this number is reported as the
 * dimension estimate.
 */
DimensionAnalysis
